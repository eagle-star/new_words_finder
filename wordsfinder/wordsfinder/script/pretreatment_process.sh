...
# 处理后增量数据（输出）
PROCESSED_NEW_DATA_PATH=${PROJECT_PATH}/feature/newdata/n_gram_
# 合并文件路劲（输入）
...
do
    ...
    spark-submit --num-executors 6 --executor-memory 4G --executor-cores 6 --conf spark.default.parallelism=500 --conf spark.storage.memoryFraction=0.5 --conf spark.shuffle.memoryFraction=0.3  $DIR/../python/pretreateddata.py $PRIMARY_HDFS_PATH $PROCESSED_NEW_DATA_PATH $NEW_AND_OLD_DATA_PATH $MERGE_DATA_PATH

    if [ $? -eq 0 ]; then
        # 替换汇总数据
        hadoop fs -rm -r $STATISTIC_DATA_PATH
        hadoop fs -mv ${PROJECT_PATH}/feature/tmp $STATISTIC_DATA_PATH
        echo "pretreatment_process is successful!"
        break
    else
